 ...
 * @version 29-01-2008
 */
public class Slider extends BuildBar
{
    ...
    private boolean showValue = true, showPercentage = false, imagesInvalid = true;
     ...
     */
    public Slider(int width, int height, String theType){
        majorSections = 2;
        minorSections = 2;
        type = theType;
        maxValue = 100;
        ...
        setImage( new GreenfootImage(actualWidth+50, height) );
    }
    ...
    public void act(){
        if(Greenfoot.mouseDragged(this) || Greenfoot.mousePressed(this)){
            ...
            int x = Math.max(valueX, mouseInfo.getX()*getWorld().getCellSize()+23);
            x -= valueX;
    ...
    private void createImages(){
        ...
        if(type.equals("Fire Rate")){
            valueWidth = actualWidth - fgWidth - valueLabelWidth - 10;
        }else{
        ...
        background = new GreenfootImage(actualWidth+50, height);
        foreground = new GreenfootImage(fgWidth, height);
        background.setColor(lineColor);
        foreground.setColor(lineColor);
    ...
    private void updateImage(){
        ...
        if(showPercentage){
            ...
            if(type.equals("Height") || type.equals("Width") || type.equals("Fuse")){
                image.drawString(type + ": "+String.valueOf(value), valueWidth+padding*2+3, image.getHeight()-3);
            }else if(type.equals("Fire Rate")){
                image.drawString(type + ": "+String.valueOf(value/5), valueWidth+padding*2+3, image.getHeight()-3);
            }else if(type.equals("AccelY")){
                image.drawString(type + ": "+String.valueOf((value-100)/100.0), valueWidth+padding*2+3, image.getHeight()-3);
            }else{
                image.drawString(type + ": "+String.valueOf(value-50), valueWidth+padding*2+3, image.getHeight()-3);
            }
        }
        setImage(image);
    }
